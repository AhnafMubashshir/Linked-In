  ...
  try {
    ...
    if (token !== 'null') {
      ...
      const authResponse = await axios.post(
        "http://host.docker.internal/users/authenticate",
        {
          headers: req.headers,
        }
      );

      console.log("Fetching posts without creator posts");

      posts = await Post.find({
        creatorID: { $ne: authResponse.data.id },
      }).sort({ createdAt: -1 });
    } else {
...
router.post("/createPost", async (req, res) => {
  ...
  try {
    ...
    const userResponse = await axios.post(
      "http://host.docker.internal/users/findUserExceptCreator",
      {
    ...
    const notificationConfirmation = await axios.post(
      "http://host.docker.internal/notifications/createNotification",
      {
